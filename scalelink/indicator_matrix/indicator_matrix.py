...
Correspondence Analysis (MCA), Scalelink starts by producing an indicator matrix
which is then used for subsequent steps. In the 2017 paper by Goldstein et al.,
the process of producing an indicator matrix is described as the process of
creating and comparing deltas.
...
def calculate_agreement_states(
    ...
    Args:
      ...
      binary_agreement_cols:
        ...
        linkage variables with binary agreement state. These columns must follow
        the naming rules outlined in the dependencies.
      df_suffixes:
    ...
    Dependencies:
      The pairs of columns containing linkage variables for the two Cartesian
        ...
        identifies which dataset the column is from. E.g., comparing a pair of
        columns in df called 'forename_df1' and 'forename_df2' would be require
        binary_agreement_cols as ['forename'] and df_suffixes as ['_df1',
        '_df2'].
    ...
    Returns:
      df_agreement:
        A dataframe consisting of df, plus an additional Boolean column for each
        pair of columns in binary_agreement_cols. The Boolean column contains
        the agreement state for the corresponding pair of linkage columns where
...
def calculate_deltas(
    ...
    Args:
      df:
        The dataframe containing the IDs, linkage variables and agreement states
        for from which the deltas will be calculated.
      cutpoints:
      ...
      agreement_col_suffix:
        The suffix that identifies agreement state columns. E.g., if the columns
        'forename_df1' and 'forename_df2' were compared and their binary
        agreement state was stored in the column 'forename_agr_state', the
        agreement_col_suffix would be '_agr_state'.
      string_similarity_suffix:
        The suffix that identifies columns containing string similarity metrics,
        e.g., Levenshtein edit distance, Sorensen-Dice coefficient. E.g., if the
        columns 'forename_df1' and 'forename_df2' were compared by calculating
        the Sorensen-Dice coefficient, with the result stored in the column
        'forename_coeff', the string_similarity_suffix would be '_coeff'.
    ...
    Dependencies:
      ...
      The agreement columns identified by agreement_col_suffix and the string
        ...
        not contain any Nulls. If Nulls are present, all deltas for that column
        and row will be false, which will cause errors downstream in the
        Scalelink method.
    ...
    Returns:
      df_with_deltas:
        ...
        variable K. Each delta column will be a Boolean column and will have a
        name with format 'di_col_name_int' where 'col_name' is the name of the
        linkage variable being compared (e.g., 'forename') and 'int' is the
        delta number.
        For columns with binary agreement state, delta 1 is True when the
        agreement state is False and delta 2 is True when the agreement state is
        True.
        For columns with partial agreement, delta 1 is True when the string
        comparison metric is equal to or greater than the bottom bound and less
        than the lowest cutpoint, delta 2 is True when the string comparison
        metric is equal to or greater than the lowest cutpoint and less than the
        next cutpoint, etc.
    """
    df_with_deltas = df
...
def calculate_sorensen_dice(
    ...
    Returns:
      df_compared:
        ...
        new_col which contains the Sorensen-Dice coefficients for the comparison
        of col1 and col2.
...
def compare_deltas(
    ...
    deltas, resulting in a comparison column for each possible pair of deltas.
    ...
    Returns:
      df_delta_comparisons:
        A dataframe containing the delta comparison columns calculated from df
        only - none of the columns derived from df are present. Each delta
        comparison column will be a non-nullable integer column and will have a
        name with format 'N_col_name1_int1_col_name2_int2' where 'col_name1' and
        'int1' refer to the first delta of the comparison (e.g., 'forename_1')
        and 'col_name2' and 'int2' refer to the second delta of the comparison.
        Delta comparisons are 1 when both deltas are True and are 0 in all other
        circumstances.
    """
...
def compute_normalised_levenshtein(
    string1: pyspark.sql.Column, string2: pyspark.sql.Column
) -> pyspark.sql.Column:
    """
    Applies the normalised Levenshtein distance to two strings, reporting a
    score normalised to between 0 and 1.

    Args:
      string1:
        First string column to be compared.
      string2:
    ...
    return levenshtein_normalised_distance
...
def get_deltas(
    ...
    linked. Also takes the input variables, stored in a dictionary. From this,
    calculates deltas (i.e., an indicator matrix) for all linkage variables.
    ...
    Returns:
      df_deltas:
        ...
        columns containing Scalelink deltas (i.e, an indicator matrix) for the
        linkage variables.
        For columns with binary agreement state, delta 1 is True when the
...
def make_bigrams(df: pyspark.sql.DataFrame, col: str) -> pyspark.sql.DataFrame:
    ...
    Returns:
      df_compared:
        ...
        character, col_bigrams will be Null.
    """
