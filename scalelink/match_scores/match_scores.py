...
match scores, given existing Scalelink deltas as defined in the 2017 paper by
Goldstein et al.
...
Methods:
  ...
  assign_weights:
    A method to calculate Scalelink match weights for each ID column pair, given
    as inputs a set of Scalelink deltas (agreement state values for ID pairs),
    and a scaled vector x* (agreement state weights by matching variable). Acts
    on the outputs of calculate_deltas, scale_x_star, and get_input_variables.
...
def assign_match_score(df_with_weights: pyspark.sql.DataFrame) -> pyspark.sql.DataFrame:
    ...
    Args:
      df_with_weights:
        ...
        joined, a row ID for the second dataset that was joined and a column for
        each Scalelink weight for each linkage variable.
...
def assign_weights(
    ...
    """
    Takes a dataframe made by Cartesian join of two datasets to be linked, with
    Scalelink deltas (an indicator matrix calculated for the agreement states of
    the linkage variables) calculated. Also takes the pre-calculated Scalelink
    weights derived from that dataframe. Returns the ID columns from that
    dataframe, plus the weights assigned to each linkage variable for each row
    (i.e., for each ID column pair).

    Args:
      df_with_deltas:
        A dataframe produced by the calculate_deltas() function.
      df1_id:
        The name of the column in df_with_deltas that contains the row ID for
        the first dataset that was joined.
      df2_id:
        The name of the column in df_with_deltas that contains the row ID for
        the second dataset that was joined.
      cutpoints:
    ...
    Returns:
      df_with_weights:
        ...
        '_weight') containing appropriate weights from x_star_scaled for each
        linkage variable and row.
    """
    # Define variables
...
def get_match_scores(
    ...
    Args:
      ...
      input_variables:
        ...
        scaling algorithm, including ID column names and string comparator cut-
        points. Dictionary keys are the names of the input variables and
        dictionary values are the values of the variables themselves. This may
        be produced by the utils function get_input_variables().
      spark:
        The Spark session to be used.
    ...
    Returns:
      df_weights_match_scores:
        ...
        weight columns (named after the linkage variables, suffixed with
        '_weight') containing appropriate weights from x_star_scaled for each
        linkage variable and row, plus a column called match_score which
