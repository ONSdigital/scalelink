...
def assign_match_score(df_with_weights):
  ...
  """
  weight_cols = [col for col in df_with_weights.columns
                    if col.endswith('_weight')]
  
  # Make match score
  df_with_match_score = df_with_weights\
  .withColumn(
    'match_score',
    F.lit(0)
  )
  ...
  for col in weight_cols:
    df_with_match_score = df_with_match_score\
    .withColumn(
    'match_score',
    F.col(col) + F.col('match_score')
  )
  
  return df_with_match_score

def assign_weights(df_with_deltas, df1_id, df2_id, cutpoints, x_star_scaled,
                   spark):
  """
  Takes a dataframe made by Cartesian join of two datasets to be linked, with
  ...
  for col in linkage_cols:
    ...
    df_with_weights = df_with_weights\
    .withColumn(
      col + '_weight',
      F.lit(None)
    )
    ...
    for key in [x for x in x_star_scaled.keys() if x.startswith(col)]:
      if '_disagree' in key:
      ...
      elif ('_agree' in key) & (cutpoints[col] == None):
        df_with_weights = df_with_weights\
        .withColumn(
          col + '_weight',
          F.when(
            F.col('di_' + col + '_2') == True,
            F.lit(x_star_scaled[key])
          )\
          .otherwise(
            F.col(col + '_weight')
        ...
        .withColumn(
          col + '_weight',
          F.when(
            F.col('di_' + col + '_' + str(len(cutpoints[col]) + 1)) == True,
            F.lit(x_star_scaled[key])
          )\
          .otherwise(
            F.col(col + '_weight')
...
def get_match_scores(df_deltas, x_star_scaled_labelled, input_variables,
                     spark):
  """
  ...
  df_weights_match_scores = msf.assign_match_score(
    df_with_weights = df_weights
  )
