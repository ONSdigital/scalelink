...
- Build method, from using `setuptools` with `setup.py`, `setup.cfg` and `requirements.txt` to using `hatchling` with
  `pyproject.toml`.
- Location of config template, so it is included in build.
- User Guide (`docs/user_guide.md`) and Contribution Guide (`docs/contribution_guide.md`), to make minor phrasing improvements.
...
- Miscellaneous CI/CD files (`.gitibnore`, `.pre-commit-config.yaml`, `.github/dependabot.yml` and whole contents of
