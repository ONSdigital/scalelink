...
def test_assign_match_score(spark):
  ...
  # Act
  test_output = msf.assign_match_score(
    df_with_weights = test_input
  )
...
def test_assign_weights(spark):
  ...
  test_input_df = spark.createDataFrame(
    [
      ('1-01', '2-01', 1,    1,    'SARAH',   'SAARAH',  True,  0.8889,
       False, True,  False, False, True),
      ('1-02', '2-02', 1,    2,    'ALEECHA', 'ALEESHA', False, 0.6667,
       True,  False, False, True,  False),
      ('1-03', '2-03', 2,    1,    'TOM',     'GRACE',   False, 0.0000,
       True,  False, True,  False, False)
    ],
    [
      'id_df1', 'id_df2', 'sex_df1', 'sex_df2', 'forename_df1', 'forename_df2',
      'sex_agr_state', 'forename_sorensen_dice', 'di_sex_1', 'di_sex_2',
      'di_forename_1', 'di_forename_2', 'di_forename_3'
    ]
  )
